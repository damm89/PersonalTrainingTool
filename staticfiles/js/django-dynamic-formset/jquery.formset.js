...
function checkRowButton() {
    if ($('.delete-row').length > 1) {
        $('.delete-row:first').removeAttr('disabled');
        $('.delete-row:first').show();
    } else {
        $('.delete-row:first').attr('disabled','true');
    ...
    $.fn.formset = function(opts)
    {
        ...
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            insertDeleteLink = function(row) {
                ...
                if (row.is('TR')) {
                    ...
                    row.append('<th scope="col" class="text-center border-top-0 px-1"><button class="' + options.deleteCssClass +'" href="javascript:void(0)">' + options.deleteText + '</button></th>');
                } else if (row.is('UL') || row.is('OL')) {
                    // If they're laid out as an ordered/unordered list,
                    // insert an <li> after the last list item:
                    row.append('<li><button class="' + options.deleteCssClass + '" href="javascript:void(0)">' + options.deleteText +'</button></li>');
                } else {
                    // Otherwise, just insert the remove button as the
                    // last child element of the form's container:
                    row.append('<button class="' + options.deleteCssClass + '" href="javascript:void(0)">' + options.deleteText +'</button>');
                }
                var delButton = row.find('button.' + delCssSelector).click(function() {
        ...
        if ($$.length) {
            ...
            if (options.formTemplate) {
                ...
                template.find('datalist:hidden[id $= "-DELETE"]').remove();
            ...
            if ($$.is('TR')) {
                ...
                addButtonRow = $('<tr><td colspan="' + numCols + '"><button class="' + options.addCssClass + '" href="javascript:void(0)">' + options.addText + '</button></tr>')
                ...
                addButton = $('<button class="my-3 ' + options.addCssClass + '" href="javascript:void(0)">' + options.addText + '</button>');
